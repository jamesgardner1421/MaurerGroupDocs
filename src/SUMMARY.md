# Summary

[MaurerGroupDocs](home.md)

# Spectroscopy

- [CASTEP XPS and NEXAFS Tutorial](spectroscopy/castep-xps-nexafs.md)

# Compilation guides

- [FHI aims on Avon](compilation/aims-avon.md)
- [FHI aims on Sulis](compilation/aims-sulis.md)
- [GPAW on Archer2](compilation/gpaw-archer2.md)
- [GPAW on Sulis](compilation/gpaw-sulis.md)

# Julia tutorials

- [Setup Julia on HPC systems](julia/julia-setup.md)
- [Contributing to NQCD](julia/nqcd.md)
- [Hosting binary packages with BinaryBuilder.jl](julia/hosting-jll.md)
- [Distributed Julia on Slurm clusters](julia/distributed-slurm.md)

# Visualisation and rendering

- [Pymol API tutorial](visualisation/pymol.md)

